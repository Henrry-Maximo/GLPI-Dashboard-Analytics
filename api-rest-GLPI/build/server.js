...
// src/http/controllers/ticketController.ts
async function ticketController(app2) {
  ...
  app2.get("/state", async (req, reply) => {
  ...
  app2.get("/amount", async (req, reply) => {
  ...
  app2.get("/tickets-line-time", async (req, reply) => {
...
async function routes(app2) {
  app2.register(ticketController, { prefix: "/tickets" });
}
...
app.listen({
  ...
  console.log(
    `\u{1F680} Server Running, port: ${env.NODE_PORT}, status: ${env.NODE_ENV}`
  );
});
